...
create table if not exists deals (
  ...
  -- Trail rows the team removed for this transaction.
  hidden_rows jsonb default '[]'::jsonb,
  workspace_domain text not null,
  created_by uuid references auth.users(id),
  created_at timestamptz default now(),
  updated_at timestamptz default now()
...
create table if not exists loi_versions (
  ...
  change_summary text,
  -- Extraction runs in a background function, so a row exists before the
  -- terms do. The browser polls status to know when the column is filled in.
